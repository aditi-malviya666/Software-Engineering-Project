...
export default function AnalysisPage() {
  ...
  const addAttempt = () =>
    setAttempts((p) => [
      ...p,
      { test_session_id: 1, is_correct: 1, time_taken_seconds: 45, topic_tag: "Calculus" },
    ]);

  const updateAttempt = (i: number, a: TestAttempt) =>
    setAttempts((p) => p.map((x, idx) => (idx === i ? a : x)));

  const removeAttempt = (i: number) =>
    setAttempts((p) => p.filter((_, idx) => idx !== i));

  return (
    <div style={{ minHeight: "100vh", background: "#060e1a", padding: "36px 16px" }}>
      <div style={{ maxWidth: 860, margin: "0 auto", fontFamily: "system-ui" }}>

        {/* HEADER */}
        <h1 style={{ color: "#f1f5f9", marginBottom: 6 }}>TCAS Model 2</h1>
        <p style={{ color: "#64748b", marginBottom: 20 }}>
          ML + AI Performance Analysis System
        </p>

        {/* INPUT CARD */}
        <div style={{ background: "#0d1f33", padding: 20, borderRadius: 12, marginBottom: 20 }}>

          {/* Student ID */}
          <div style={{ display: "flex", gap: 10, marginBottom: 15 }}>
            <label style={{ color: "#94a3b8" }}>Student ID:</label>
            <input
              type="number"
              value={studentId}
              onChange={(e) => setStudentId(Number(e.target.value))}
            ...
            <span style={{ marginLeft: "auto", color: "#64748b" }}>
              Sessions: {sessionCount} {sessionsNeeded > 0 && `(need ${sessionsNeeded})`}
            </span>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {attempts.map((a, i) => (
              <div key={i} style={{ display: "flex", gap: 8 }}>
                ...
                <input
                  type="number"
                  value={a.test_session_id}
                  onChange={(e) =>
                    updateAttempt(i, { ...a, test_session_id: Number(e.target.value) })
                  }
                  style={{ flex: 1 }}
                />
                ...
                <select
                  value={a.topic_tag}
                  onChange={(e) =>
                    updateAttempt(i, { ...a, topic_tag: e.target.value })
                  }
                  style={{ flex: 2 }}
                >
                  {TOPICS.map((t) => (
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateAttempt(i, {
                      ...a,
                      is_correct: Number(e.target.value) as 0 | 1,
                    })
                  }
                  style={{ flex: 1 }}
                >
                ...
                <input
                  type="number"
                  value={a.time_taken_seconds}
                  onChange={(e) =>
                    updateAttempt(i, {
                      ...a,
                      time_taken_seconds: Number(e.target.value),
                    })
                  }
                  style={{ flex: 1 }}
                />

                <button onClick={() => removeAttempt(i)}>❌</button>
              </div>
            ))}
          </div>
              ...
              onClick={() =>
                analyse({
                  ...
                  attempts: attempts.map((a) => ({
                    ...a,
                    is_correct: Number(a.is_correct) as 0 | 1,
                  })),
                })
              }
            >
        ...
        {/* RESULT */}
        {data && ls && (
          <div style={{ background: "#0d1f33", padding: 20, borderRadius: 12 }}>

            <h3 style={{ color: "#fff" }}>
              {ls.icon} {data.rf_label}
            </h3>
            ...
            <p style={{ color: "#94a3b8" }}>
              Latency: {Math.round(data.moving_avg_latency)}s
            </p>

            {/* Topics */}
            <div style={{ marginTop: 15 }}>
              {Object.entries(data.topic_accuracies).map(([topic, acc]) => (
                <div key={topic} style={{ marginBottom: 8 }}>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span>{topic}</span>
                    <span style={{ color: barColor(acc) }}>
                      {Math.round(acc * 100)}%
                    </span>
                  </div>

                  <div style={{ height: 6, background: "#111", borderRadius: 5 }}>
                    <div
                      style={{
                        width: `${acc * 100}%`,
                        height: "100%",
                        background: barColor(acc),
                      }}
                    />
                  </div>
                </div>
